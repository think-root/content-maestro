...
func createTablesIfNotExist(db *sql.DB) error {
	...
	_, err = db.Exec(`
		CREATE TABLE IF NOT EXISTS think_prompt (
			...
			llm_output_language VARCHAR(10) NOT NULL DEFAULT 'en,uk',
			updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
		)`)
	if err != nil {
	...
	_, err = db.Exec(`
		INSERT INTO think_prompt (use_direct_url, llm_provider, temperature, content, model, llm_output_language, updated_at)
